...
@Singleton
public class JsoupWebPageCrawler implements WebPageCrawler {
	...
	@Inject
	public JsoupWebPageCrawler() {
	}

	private final String HTTPS_PROTOCOL = "https://";

	private SourceContent fetchSourceContent(String url, int imageQuantity) throws LinkPreviewException {
		...
		if (!sourceContent.getFinalUrl().equals("")) {
			if (isImage(sourceContent.getFinalUrl())
				...
				try {
					...
					throw new LinkPreviewException("failed to get link preview", e);
				}
			}
		}
		...
		if (extendedTrim(sourceContent.getHtmlCode()).equals("")
				&& !isImage(sourceContent.getFinalUrl())) {
			throw new LinkPreviewException("is null", null);
		}
	...
	@Override
	public SourceContent makePreview(final String url) throws LinkPreviewException {
		return fetchSourceContent(url, 1);
	}
}
